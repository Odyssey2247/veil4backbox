...
# Check Sudo Dependency
    ...
    echo '                For blackarch : use with sudo'
    echo ''
    exit 1
    fi
...
func_apt_deps(){
  ...
  # Start Dependency Install
    ...
    if [ "${os}" == "ubuntu" ] || [ "${os}" == "debian" ] || [ "${os}" == "kali" ]; then
  # mod for backbox add winbind---------------------------------------------------------------------------------------
    sudo ${arg} apt-get -y install mingw-w64 monodoc-browser monodevelop mono-mcs wine unzip ruby golang wget git \
                          python python-crypto python-pefile python-pip ca-certificates ttf-mscorefonts-installer winbind
    tmp="$?"
   ...
   # mod for blackarch--------------------------------------------------------------------------------------------------
    ...
    sudo ${arg} pacman -S mingw-w64-binutils mono-tools monodevelop go git python-pip python2-pip 
    tmp="$?"
        [ "${tmp}" -ne "0" ] && echo -e " [ERROR] Failed To Install APT Dependencies (Metasploit)... Exit Code: ${tmp}.\n" && exit 1
    fi
}
...
func_capstone_deps(){
   ...
   # mod for blackarch--------------------------------------------------------------------------------------------------
    if [ "${os}" == "blackarch" ]; then
    sudo ${arg} pacman -S python-capstone
    tmp="$?"
        [ "${tmp}" -ne "0" ] && echo -e " [ERROR] Failed To Install Capstone Dependencies... Exit Code: ${tmp}.\n" && exit 1
else
...
func_python_deps(){
  ...
  # Check If SymmetricJSONRPC Is Already Installed
    ...
    # mod for blackarch---pip to pip2.7---------------------------------------------------------------------------------
    sudo pip2.7 install symmetricjsonrpc
    tmp="$?"
        [ "${tmp}" -ne "0" ] && echo -e " [ERROR] Failed To Install SymmetricJSONRPC... Exit Code: ${tmp}.\n" && exit 1
    echo ''
    fi
...
echo " [i] blackarch ${version} $(uname -m) Detected..."
elif [ "${os}" == "kali" ]; then
